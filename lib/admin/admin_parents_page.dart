                                  ...
                                  : () async {
                                      ...
                                      try {
                                        ...
                                        for (final parentDoc in parentsSnap.docs) {
                                          if (parentDoc.id == parentUid) continue;
                                        ...
                                        final previousChildIds = currentChildIds.toSet();
                                        final nextChildIds = selectedChildIds.toSet();
                                        final removedChildIds = previousChildIds.difference(nextChildIds);
                                        final addedChildIds = nextChildIds.difference(previousChildIds);
                                        final batch = FirebaseFirestore.instance.batch();
                                        final parentRef = FirebaseFirestore.instance
                                            .collection('users')
                                            .doc(parentUid);

                                        batch.update(parentRef, {'children': selectedChildIds});

                                        for (final studentUid in removedChildIds) {
                                          final studentRef = FirebaseFirestore.instance
                                              .collection('users')
                                              .doc(studentUid);
                                          batch.update(studentRef, {
                                            'parents': FieldValue.arrayRemove([parentUid]),
                                          });
                                        }

                                        for (final studentUid in addedChildIds) {
                                          final studentRef = FirebaseFirestore.instance
                                              .collection('users')
                                              .doc(studentUid);
